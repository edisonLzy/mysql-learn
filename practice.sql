...
-- 查询每个用户的订单总额,按照订单总额降序排列,只显示前 3 条记录
-- 如果在 SQL 查询中使用了聚合函数，比如 SUM、AVG、COUNT、MAX 或 MIN，那么就需要使用 GROUP BY 子句来对查询结果进行分组
SELECT name, SUM(total_amount) AS total_amount FROM customers JOIN orders ON customers.id = orders.customer_id GROUP BY customers.id ORDER BY total_amount DESC LIMIT 0,3;

-- 查询每一个用户订单总额的总占比
SELECT name, SUM(total_amount) AS total_amount , SUM(orders.total_amount) / (SELECT SUM(total_amount) from orders) AS percentage 
FROM customers JOIN orders ON customers.id = orders.customer_id GROUP BY customers.id ORDER BY total_amount DESC;